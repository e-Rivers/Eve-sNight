         ...
         function(stdout) 
             if (tonumber(stdout) ~= nil) then
                ...
                if (tonumber(stdout) > 50) then
                    volume_icon.image=gears.surface.load_uncached(icons.png.vol_inc)
                elseif (tonumber(stdout) > 0) then
                    volume_icon.image=gears.surface.load_uncached(icons.png.vol_dec)
                else
                    volume_icon.image=gears.surface.load_uncached(icons.png.vol_mute)
               end
            else
               volume_bar.value = 0
               volume_icon.image=gears.surface.load_uncached(icons.png.vol_mute)
